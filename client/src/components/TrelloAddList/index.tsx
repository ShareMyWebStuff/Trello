...
export default function TrelloAddList ( {}) {
    ...
    return (<div  onClick={()=> {setIsFocused(true)}} className="TrelloAddList-container" data-test="addList">
        <form>
            { isFocused ? <div><input autoFocus type="text" placeholder="Enter list" value={listName} onChange={(e) =>setListName (e.target.value)} ></input>
            <div className=''>
                <button className="add-btn" onClick={(e) => {e.preventDefault(); saveList()}} >Add list</button>
